...
instance of `BB` using `create bf.f`. But then we call `bf.h` which calls the once procedure `bf.g`
before creating the second instance of `BB`. When getting the second instance of `BB` using
`create bg.g`, the call to `g` has no effect since this once procedure has already been called.
...
* ISE Eiffel (as of 25.12.9.8922 and after) lets `bf` wrongly point to the second instance of
  `BB` when calling `bf.h` (and indirectly `bf.g`), as if the instruction was `create bf.g`.
