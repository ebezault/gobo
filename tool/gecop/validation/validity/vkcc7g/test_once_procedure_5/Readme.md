...
instance of `BB` using `create bf.f`. Then we call `bf.h` which calls another once
procedure `bf.g`. In this test `VKCC-7G` is not violated because `f` has been
declared in class `BB`.
