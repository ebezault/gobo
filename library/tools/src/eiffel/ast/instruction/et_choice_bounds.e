...
feature {NONE} -- Initialization
	...
	make (a_choice: like choice; a_lower_value: NATURAL_64; a_is_lower_negative: BOOLEAN; a_upper_value: NATURAL_64; a_is_upper_negative: BOOLEAN)
	...
	make_with_lower_value (a_choice: like choice; a_lower_value: NATURAL_64; a_is_lower_negative: BOOLEAN)
			-- Create a new choice bounds.
		require
			a_choice_not_void: a_choice /= Void
		do
	...
	make_with_lower_integer_constant (a_choice: like choice; a_integer_constant: ET_INTEGER_CONSTANT)
			-- Create a new choice bounds.
		require
			a_choice_not_void: a_choice /= Void
	...
	make_with_lower_character_constant (a_choice: like choice; a_character_constant: ET_CHARACTER_CONSTANT)
			-- Create a new choice bounds.
		require
			a_choice_not_void: a_choice /= Void
...
feature -- Status report
	...
	overlaps (other: ET_CHOICE_BOUNDS): BOOLEAN
		...
		do
			if other.is_lower_negative then
				if is_upper_negative then
					Result := other.lower_value >= upper_value
				else
					Result :=  other.lower_value = 0 and upper_value = 0
				end
			elseif is_upper_negative then
				Result := other.lower_value = 0 and upper_value = 0
			else
			...
			elseif is_lower_negative then
				if other.is_upper_negative then
					Result := lower_value >= other.upper_value
				else
					Result := lower_value = 0 and other.upper_value = 0
				end
			elseif other.is_upper_negative then
				Result :=  lower_value = 0 and other.upper_value = 0
			else
				Result := lower_value <= other.upper_value
			end
...
feature -- Access
	...
	choice: ET_CHOICE
			-- Choice constant or choice interval
...
feature -- Setting
	...
	set_choice (a_choice: like choice)
			-- Set `choice` to `a_choice`.
		require
			a_choice_not_void: a_choice /= Void
		do
			choice := a_choice
